...
/**
 * GET /api/brand/messages/conversations/[id]/messages
...
export async function GET(
  ...
  try {
    ...
    const enrichedMessages = await Promise.all(
      messages.map(async (msg) => {
        ...
        return {
          ...
          content: msg.content,
          readAt: msg.readBy?.brand ? msg.readBy.brand.toISOString() : null,
    ...
    // Mark conversation as read for brand
    if (conversation.unreadCount.brand > 0) {
...
/**
 * POST /api/brand/messages/conversations/[id]/messages
...
export async function POST(
  ...
  try {
    ...
    const message = await Message.create({
      ...
      readBy: {
        brand: new Date(), // Brand has read their own message
      },
    })
    ...
    return NextResponse.json({
      ...
      message: {
        ...
        senderName: sender?.fullName || 'Brand',
